...
import { CONSTANTS } from './src/config/contants.js';

import { loadTexture, loadOBJ } from './src/utils/loader.js';
import { getCanvasElement, getControlsElements } from './src/config/elements.js';
...
async function main() {
    ...
    // let MOUSE_ROTATION_SPEED, MAX_LAYERS, MAX_CLOUD_LAYERS, MAX_CLOUD_OFFSET;
    // let MIN_PHI, MAX_PHI, AUTO_ROTATE, LEFT_BUTTON_VALUE;
    // let PLANET_ROTATION_SPEED, MOVE_ACCELERATION, TURN_ACCELERATION;
    // let FRICTION_MOVE, FRICTION_TURN;

    // let numActiveLayers, numActiveCloudLayers, cloudLayerOffset;
    // let isMouseOverUI, showWireframe, showLambertianDiffuse, showClouds, topDownMode;

    // let noiseParams, camera, layerLevels, layerColors;
    // let shadersParams, cloudParams, cloudShadowParams;
    ...
    //     numActiveLayers         = DEFAULTS.app.numActiveLayers;
    //     numActiveCloudLayers    = DEFAULTS.app.numActiveCloudLayers;
    //     cloudLayerOffset        = DEFAULTS.app.cloudLayerOffset;
    //     isMouseOverUI           = DEFAULTS.app.isMouseOverUI;
    //     showWireframe           = DEFAULTS.app.showWireframe;
    //     showLambertianDiffuse   = DEFAULTS.app.showLambertianDiffuse;
    //     showClouds              = DEFAULTS.app.showClouds;
    //     topDownMode             = DEFAULTS.app.topDownMode;

    //     noiseParams       = { ...DEFAULTS.noise };
    //     camera            = { ...DEFAULTS.camera };
    //     layerLevels       = { ...DEFAULTS.layerLevels };
    //     layerColors       = { ...DEFAULTS.layerColors };
    //     shadersParams     = { ...DEFAULTS.shaders };
    //     cloudParams       = { ...DEFAULTS.clouds };
    //     cloudShadowParams = { ...DEFAULTS.cloudShadowParams };
    // }
    ...
    const planetRotationMatrix = mat4.create();
    ...
    function updatePlanetPhysics() {
        ...
        if (keys.a) rotationVelocityY += TURN_ACCELERATION; 
        if (keys.d) rotationVelocityY -= TURN_ACCELERATION;

        rotationVelocityX *= FRICTION_MOVE;
        rotationVelocityY *= FRICTION_TURN;

        const rotationStep = mat4.create();
        mat4.identity(rotationStep);
        
        mat4.rotateX(rotationStep, rotationStep, rotationVelocityX);
        mat4.rotateY(rotationStep, rotationStep, rotationVelocityY);

        mat4.multiply(planetRotationMatrix, rotationStep, planetRotationMatrix);
        
        const rollIntensity = 15; 
        
        const currentRoll = rotationVelocityY * rollIntensity; 
        plane.rotationOffset = [
            baseRotationOffset[0] + Math.abs(currentRoll),
            baseRotationOffset[1] + currentRoll, 
            baseRotationOffset[2] 
        ];
    }
    ...
    function handleMouseDown(event) {
        if (isMouseOverUI) return;
        if (event.button !== LEFT_BUTTON_VALUE) return; 
    ...
    function handleAnimation() {
        ...
        const rotationMatrixToUse = topDownMode ? planetRotationMatrix : null;
        ...
        if (animation.isAnimating) {
            ...
            animation.currentScale = animation.startScale + (animation.targetScale - animation.startScale) * easedProgress;
        ...
        if (cloudsToggle.checked){
            //SOMBRA DAS NUVENS
            renderer.render(time, cameraPosition, cloudShadowParams, showWireframe, showLambertianDiffuse, AUTO_ROTATE, 3., rotationMatrixToUse);
            //NUVENS
            for (let i = 0; i < numActiveCloudLayers; i++) {
                ...
                renderer.render(time, cameraPosition, layerParams, showWireframe, showLambertianDiffuse, AUTO_ROTATE, 2., rotationMatrixToUse);
            }
        }
